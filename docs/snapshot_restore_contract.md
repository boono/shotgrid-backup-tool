...
媒体任务必须把运行绑定到启动时已经存在的输出目录身份。输出卷消失、设备/inode 身份变化或底层返回断挂 I/O 错误时，所有传输池必须立即 fail closed，并保留可续传 `.incomplete`；不得继续批量制造失败项，也不得在消失的挂载点下静默创建同名本地目录。失败 index 与 `logs/errors.json` 应保留脱敏、限长的错误码、异常类型和说明，但不得保存签名 URL、API Key、代理凭据或绝对路径。
